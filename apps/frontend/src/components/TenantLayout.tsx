...
const TenantLayout: React.FC = () => {
  ...
  const handleSidebarToggle = () => {
    ...
    localStorage.setItem('tenantSidebarCollapsed', JSON.stringify(newState));
  };

  // Navegación específica según el rol del usuario
  const navigation = user?.role === 'doctor' ? [
    { name: 'Dashboard', href: `/${tenant?.domain}/doctor/dashboard`, icon: Activity, description: 'Vista general médica' },
    { name: 'Mis Citas', href: `/${tenant?.domain}/doctor/citas`, icon: Calendar, description: 'Gestión de citas médicas' },
    { name: 'Calendario', href: `/${tenant?.domain}/doctor/calendario`, icon: Calendar, description: 'Calendario de disponibilidad' },
    { name: 'Disponibilidad', href: `/${tenant?.domain}/doctor/disponibilidad`, icon: Clock, description: 'Gestionar horarios' },
    { name: 'Mi Perfil', href: `/${tenant?.domain}/doctor/perfil`, icon: User, description: 'Perfil profesional' },
  ] : [
    { name: 'Dashboard', href: `/${tenant?.domain}/dashboard`, icon: Home, description: 'Vista general de la institución' },
    { name: 'Pacientes', href: `/${tenant?.domain}/patients`, icon: Users, description: 'Gestión de pacientes' },
    { name: 'Personalización', href: `/${tenant?.domain}/personalization`, icon: Settings, description: 'Configuración de hospitales, doctores y seguros' },
    { name: 'Suplantar Doctores', href: `/${tenant?.domain}/doctor-impersonation`, icon: UserCheck, description: 'Ver el sistema como doctor' },
    { name: 'Citas agendadas', href: `/${tenant?.domain}/appointments`, icon: Calendar, description: 'Gestión de citas médicas' },
    { name: 'Configuración', href: `/${tenant?.domain}/settings`, icon: Settings, description: 'Ajustes del sistema' },
  ];
        ...
        <div className="fixed inset-y-0 left-0 flex w-80 flex-col bg-white shadow-2xl">
          <div className="flex h-20 items-center justify-between px-6 border-b border-neutral-100">
            <div className="flex items-center space-x-3">
              ...
              <h1 className="text-lg font-bold text-neutral-900 truncate">{tenant.name}</h1>
            </div>
            <button
        ...
        <div className="flex flex-col flex-grow bg-white border-r border-neutral-100 shadow-lg">
          ...
          <div className="flex items-center h-20 px-6 border-b border-neutral-100">
            <div className="flex items-center space-x-3">
              ...
              <h1 className={`text-lg font-bold text-neutral-900 transition-all duration-300 truncate ${
          ...
          <nav className={`flex-1 px-4 ${
            sidebarCollapsed ? 'py-2 space-y-0.5' : 'py-6 space-y-2'
          }`}>
            {navigation.map((item) => {
              const Icon = item.icon;
              return (
                <Link
                  ...
                  className={`group flex items-center text-sm font-medium rounded-xl transition-all duration-200 ${
                    sidebarCollapsed ? 'px-1 py-1.5 justify-center' : 'px-4 py-3 justify-start'
                  } ${
                    isActive(item.href)
                      ? 'bg-primary-50 text-primary-700 border border-primary-200 shadow-sm'
                      : 'text-neutral-600 hover:bg-neutral-50 hover:text-neutral-900'
                ...
                >
                  <Icon className={`transition-all duration-300 ${
                    sidebarCollapsed ? 'h-4 w-4 mx-auto' : 'h-5 w-5 mr-3'
                  }`} />
            ...
            <div className={`transition-all duration-300 ${
              ...
              <div className="p-3 bg-gradient-to-r from-primary-50 to-medical-50 rounded-xl border border-primary-100">
                <div className="flex items-center space-x-2 text-xs text-primary-700">
                  ...
                  <span className="font-medium">Institución Activa</span>
                </div>
        ...
        <main className="py-8">
          <div className="mx-auto max-w-7xl px-6 sm:px-8">
            <Outlet />
             ...
             {/* Panel de Notificaciones - Solo para administradores */}
       {user?.role !== 'doctor' && (
         <NotificationPanel
