...
import type { VerifyCodeResponse } from '../services/twoFactorService';
...
const TwoFactorVerification: React.FC = () => {
  ...
  const handleVerify = async (verificationCode: string) => {
    ...
    try {
      const response: VerifyCodeResponse = await twoFactorService.verifyCode(currentTempToken, verificationCode);
  ...
  const handleResendCode = async () => {
    if (!currentTempToken || resendCooldown > 0) return;
    
    setIsLoading(true);
    setError('');

    try {
      const response = await twoFactorService.resendCode(currentTempToken);
